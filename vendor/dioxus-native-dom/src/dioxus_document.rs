    ...
    fn handle_event(
        ...
        let event_data = match &event.data {
            ...
            | DomEventData::Click(mevent) => {
                // Element relative coordinates are not on the blitz event, but
                // the target's box is known here: ask the document where the
                // node sits and hand the offset along. Without it,
                // `element_coordinates` panics, which takes the whole
                // application down on any handler that reads where a click
                // landed within the thing it landed on.
                //
                // `absolute_position` already accounts for scrolling, so a
                // click inside a scrolled pane is measured from the element
                // rather than from where the element would have been.
                let origin = mutr
                    .doc
                    ...
                    // A target that has gone between the click and the
                    // dispatch leaves the coordinates as the client ones,
                    // which is wrong by the element's offset and still better
                    // than bringing the process down.
                    .unwrap_or((0.0, 0.0));
                Some(wrap_event_data(NativeClickData(mevent.clone(), origin)))
            }
