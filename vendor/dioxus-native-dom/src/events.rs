...
pub struct NativeClickData(
    pub(crate) BlitzMouseButtonEvent,
    /// Where the target element's border box begins, in the same space the
    /// event's own coordinates are given in.
    ///
    /// The blitz event carries only the client point. Element relative
    /// coordinates are that point measured from here, and the subtraction has
    /// to happen where the document is still in reach, which is the dispatcher
...
impl InteractionElementOffset for NativeClickData {
    fn element_coordinates(&self) -> ElementPoint {
        let (left, top) = self.1;
        ElementPoint::new((self.0.x - left) as _, (self.0.y - top) as _)
    }
}
...
#[cfg(test)]
mod element_offset_tests {
    use super::*;
    use blitz_traits::events::{MouseEventButton, MouseEventButtons};

    fn click_at(x: f32, y: f32, origin: (f32, f32)) -> NativeClickData {
        NativeClickData(
    ...
    #[test]
    fn element_coordinates_are_measured_from_the_element() {
        // A click 100 across and 50 down the window, on an element whose box
        // starts 30 across and 20 down, landed 70 and 30 into that element.
        let point = click_at(100.0, 50.0, (30.0, 20.0)).element_coordinates();
        assert_eq!(point.x, 70.0);
        assert_eq!(point.y, 30.0);
    }

    #[test]
    fn a_click_on_the_corner_is_the_origin() {
        let point = click_at(30.0, 20.0, (30.0, 20.0)).element_coordinates();
        assert_eq!(point.x, 0.0);
        assert_eq!(point.y, 0.0);
    }

    #[test]
    fn an_unknown_target_falls_back_rather_than_panicking() {
        // The dispatcher passes a zero origin when the target has gone. That
        // is wrong by the element's offset and it is not a crash, which is the
        // entire point: before this, reading the coordinates at all brought
        // the process down.
        let point = click_at(100.0, 50.0, (0.0, 0.0)).element_coordinates();
        assert_eq!(point.x, 100.0);
        assert_eq!(point.y, 50.0);
    }

    #[test]
    fn the_client_point_is_left_alone() {
        let data = click_at(100.0, 50.0, (30.0, 20.0));
        let client = data.client_coordinates();
        assert_eq!(client.x, 100.0);
        assert_eq!(client.y, 50.0);
    }
}
