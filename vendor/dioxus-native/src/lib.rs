...
pub fn launch_cfg_with_props<P: Clone + 'static, M: 'static>(
    ...
    // Any font the program carries is registered before the document exists,
    // so the first layout already has it. `None` when there are none, which is
    // the default and leaves blitz to build its own from the system.
    let font_ctx = if config.fonts.is_empty() {
        None
    } else {
        let mut ctx = parley::FontContext::new();
        for font in &config.fonts {
            ctx.collection.register_fonts(
                parley::fontique::Blob::new(std::sync::Arc::new(*font)),
                None,
            );
        }
        Some(ctx)
